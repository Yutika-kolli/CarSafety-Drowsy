...
def eyes_cnnmodel():
    print("[Info] Start to Training the CNN model for Eyes...")
    BS= 32
    TS=(24,24)
    ...
    plt.title('Training and Validation accurarcy for Eyes dataset')
    plt.legend()
    plt.figure()
    # Train and validation loss
    plt.plot(epochs, loss, 'b', label='Training loss')
    plt.plot(epochs, val_loss, 'r', label='Validation loss')
    plt.title('Training and Validation loss for Eyes dataset')
    plt.legend()
    plt.show()

    model.save('models/cnn_eyes.h5', overwrite=True)

    print("[Info] Training the CNN model for Eyes Completed.")
#eyes_cnnmodel()